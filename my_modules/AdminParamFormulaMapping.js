...
class AdminParamFormulaMappingVO extends BaseClass.BaseVO {
    constructor() {
        super()
        this._ID = 0
        this._ID_NCRAutoAdminTopic = 0
        this._ID_ParamFormulaMapping = 0
        this._ParamName = ''
        this._ParamType = ''
        this._IsConst = 0
        this._ConstVal = ''
    ...
    get ID_ParamFormulaMapping() {
        return parseInt(this._ID_ParamFormulaMapping);
    }
    set ID_ParamFormulaMapping(value) {
        this._ID_ParamFormulaMapping = parseInt(value)
    }
...
class AdminParamFormulaMappingEXE extends BaseClass.BaseEXE {
    constructor(connDetail) {
        super(connDetail)
        this.result = new AdminParamFormulaMappingVO()
    }
    ...
    async search(DataVO) {
        try {
            ...
            let request = await pool.request()
                .input('ID_NCRAutoAdminTopic', DataVO.ID_NCRAutoAdminTopic)
                .input('ID_ParamFormulaMapping', DataVO.ID_ParamFormulaMapping)
                .input('ParamName', DataVO.ParamName)
                .input('ParamType', DataVO.ParamType)
                .input('IsConst', DataVO.IsConst)
                .input('ConstVal', DataVO.ConstVal)
                .input('Active', DataVO.Active)
                .execute('admin_param_formula_mapping_search');
            this.dataSet = request.recordsets[0];
            // console.log('dataset---------------> ', this.dataSet)
        }
        catch (error) {
            this.logErrorExec('****** Error admin_param_formula_mapping_search : ' + error + '******')
            this.dataSet = []
        }
    ...
    async listby_topic(ID_NCRAutoAdminTopic) {
        try {
            let pool = await this.db.connect(this.connDetail);
            let request = await pool.request()
                .input('ID_NCRAutoAdminTopic', ID_NCRAutoAdminTopic)
...
module.exports.AdminParamFormulaMappingVO = AdminParamFormulaMappingVO
module.exports.AdminParamFormulaMappingEXE = AdminParamFormulaMappingEXE
